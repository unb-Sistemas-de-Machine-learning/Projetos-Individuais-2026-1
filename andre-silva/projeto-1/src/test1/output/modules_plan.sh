...
class Parser():
    ...
    def parse(self):
        ...
        if self.current_token.tokenType != TokenType.EOF:
            raise ValueError(f"Error while parsing string {tokenizer.input_str}")

    def __expression(self):
        self.__term()
        if self.current_token == TokenType.OR:
            self.check_token(TokenType.OR)
            self.__term()

    def __term(self):
        self.__factor()
        if self.current_token == TokenType.AND:
